...
class Dynamics2D : public Lattice2D {
  ...
  public:
    Dynamics2D(int nx_, int ny_, int nz_, int q_, int decomposition_[3], int procCoordinates_[3], vector<double> origin_, vector<double> boxLength_, int dimension_, double dx_);
    ~Dynamics2D();
