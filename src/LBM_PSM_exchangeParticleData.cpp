...
void ExchangeParticleData::setParticlesOnLattice(LBMPSMLattice *lattice_, UnitConversion *unitConversion, int numberParticles, LAMMPS_NS::tagint *tag, double **xPart, double **uPart, double **omega, double *rp, vector<double> boxLength, vector<double> origin)
{
  for(int iPart = 0; iPart < numberParticles; ++iPart){
      ...
      double x_lb_local = x_lb_global - (double)lattice_->get_procCoordinates()[0]*((double)lattice_->get_nx()-2.0*(double)lattice_->get_envelopeWidth());
      double y_lb_local = y_lb_global - (double)lattice_->get_procCoordinates()[1]*((double)lattice_->get_ny()-2.0*(double)lattice_->get_envelopeWidth());
      double z_lb_local = z_lb_global - (double)lattice_->get_procCoordinates()[2]*((double)lattice_->get_nz()-2.0*(double)lattice_->get_envelopeWidth());
      ...
      for(int i = nodeZone[0][0]; i < nodeZone[0][1]; ++i){
        for(int j = nodeZone[1][0]; j < nodeZone[1][1]; ++j){
          for(int k = nodeZone[2][0]; k < nodeZone[2][1]; ++k){
            ...
            if (tag[iPart] == lattice_->getParticleDataOnLatticeNode(ind_phys_1D).particleID[1]){
            ...
            switch(decFlag){
              ...
              case 1: // sf > 0 && sf_old == 0
                lattice_->setParticleOnLattice(ind_phys_1D, tag[iPart], uNode, calcSolidFraction(i, j, k, x_lb_local, y_lb_local, z_lb_local, r_lb));
                break;
...
void ExchangeParticleData::calculateHydrodynamicInteractions(LBMPSMLattice *lattice_, UnitConversion *unitConversion, LAMMPS_NS::tagint tag, double *xPart, double rp, vector<double> &fHydro, vector<double> &tHydro, vector<double> &stresslet)
{
    ...
    for (int i = 0; i < 2; i++){
        ...
        nodeZone[2][i] = fmin(fmax(nodeZone[2][i],envelopeWidth),lattice_->get_nz()-envelopeWidth);
    }
