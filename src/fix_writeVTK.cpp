...
void WriteVTK::pre_force(int)
{
  ...
  double ly = 1.0;
  write_vtk("run.vtk", x_vtk, 1.0/ly, y_vtk, 1.0/ly, B_vtk, 1.0, rho_vtk, 1000.0, u_vtk, 1.0/u_infty);
}
...
void WriteVTK::write_vtk(string name_, vector<double> &x_, vector<double> &y_, vector<double> &B_, vector<double> &rho_, vector<double> &u_){
...
vector<double> point_id(nx*decomposition[0]*ny*decomposition[1], 0.0);
...
// POINTS
int pnt_id = -1;
ovel << "POINTS " << nx*decomposition[0]*ny*decomposition[1] << " FLOAT\n";
for(int i=0; i<nx*decomposition[0]; i++){
  for(int j=0; j<ny*decomposition[1]; j++){
    ovel << x_[i*ny*decomposition[1]+j] << " " << y_[i*ny*decomposition[1]+j] << " 0" << "\n";
    pnt_id += 1;
    point_id[i*ny*decomposition[1]+j] = pnt_id;
  }
}
// CELLS
...
void WriteVTK::write_vtk(string name_, vector<double> &x_, double x0_, vector<double> &y_, double y0_, vector<double> &B_, double B0_, vector<double> &rho_, double rho0_, vector<double> &u_, double u0_){
  ...
  vector<double> u0;
  if(comm->me == 0){
    x0.resize(nx*decomposition[0]*ny*decomposition[1]);
    y0.resize(nx*decomposition[0]*ny*decomposition[1]);
    B0.resize(nx*decomposition[0]*ny*decomposition[1]);
...
//  MPI_Gather(&u_[0], nx*ny*2, MPI_DOUBLE, &u0[0], nx*ny*2, MPI_DOUBLE, 0, mpicomm.cartComm3D);
  ...
  MPI_Gather(&x_[0], nx*ny, MPI_DOUBLE, &x0[0], nx*ny, MPI_DOUBLE, 0, world);
  MPI_Gather(&y_[0], nx*ny, MPI_DOUBLE, &y0[0], nx*ny, MPI_DOUBLE, 0, world);
